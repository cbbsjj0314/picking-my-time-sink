...
- **Mapping boundary**: `docs/decisions/category-to-game-mapping-contract.md`에 category-to-game mapping의 planning boundary만 문서화했으며, implementation, automatic category matching, trusted Combined usage, Combined semantics는 아직 구현하지 않았다.
...
| Chzzk observed source API | Partially implemented | category/channel observed facts와 read-only `/chzzk/categories/overview` |
| Chzzk observed source view | Bounded implemented | category-only observed evidence browser backed by `/chzzk/categories/overview`; not a Steam-equivalent product baseline |
| Mapping / Combined KPI | Not implemented | category-to-game mapping contract는 planning boundary이며, Steam-Chzzk mapping과 Combined KPI는 future work |
...
### Chzzk Source-View 구현 범위

Chzzk observed source view는 bounded implemented 상태의 category-only observed evidence browser다.

API 및 웹의 bounded source-view 기능은 `/chzzk/categories/overview`를 기준으로 제한된 범위에서 구현했으나, Steam-only baseline 수준의 완성된 사용 가능 베이스라인으로 간주하지는 않는다.
...
정기 수집/write-path 안정화, category-to-game mapping, Combined source/KPI system은 향후 과제로 남겨두었다.
...
* **Web source views**: React와 Vite 기반의 대시보드로 Steam의 `Explore` 및 `Top Selling` 화면을 제공하며, Chzzk의 관측된 카테고리 source view는 bounded category-only evidence browser로 제공한다.
...
* Chzzk regular collection/write-path stabilization
* category-to-game mapping
* Combined source/KPI system

...
- 현재 repo 관찰:
    ...
    - Chzzk local/private bounded probe wrapper/scheduler evidence 자체는 product/runtime integration이 아니다.
      현재 repo에는 read-only `/chzzk/categories/overview` 와 web `ChzzkCategoryTable` 기반 observed source-view가 구현되어 있지만,
      이는 Steam-equivalent completed baseline, trusted Steam mapping, Combined KPI가 아니다.
...
- 첫 provider-specific source-view (현재 구현):
    - 첫 provider-specific source-view는 Chzzk category evidence browser로 구현되어 있다.
    ...
    - category evidence는 category-only observed source-view로만 해석한다. `categoryType=GAME` 이 있어도 canonical game semantics, Steam mapping, API/UI game column, Combined semantics로 확장하지 않는다.
    - category-level 30분 fact candidate 방향은 `(chzzk_category_id, bucket_time)` 단위의 category type/name, concurrent 합계, live_count, top channel evidence다.
    ...
    - runtime ingest 책임은 derived/sanitized `category-result.jsonl` 을 category 30분 row로 적재하는 데 그친다.
      API serving은 read-only category overview에 한정하며, web source-view는 observed category evidence boundary만 노출한다.
      canonical game mapping, Combined semantics는 만들지 않는다.
...
## 2. Chzzk (category evidence browser; observed source-view 구현, real integration 미착수)

### 2.1 Live 목록

- 목적: category-only 시청/방송 evidence browser 후보 원천
...
- product contract:
    - 첫 Chzzk source-view는 category evidence browser로 구현되어 있다.
    ...
    - 현재 repo에는 `fact_chzzk_category_30m` artifact-to-Postgres write path와
      ...
      `ChzzkCategoryTable` 기반 observed source-view는 read-only boundary로 제공한다.
...
- 실패/주의:
    ...
    - category result는 category-only observed source-view boundary로만 읽는다. `/chzzk/categories/overview` 는 category-only observed sample serving semantics만 열고, category-to-game mapping, UI serving semantics, Combined semantics는 열지 않는다.
...
- Updated by CATEGORY-MAPPING-COMBINED-BACKEND-API-CONTRACT-001: the selected first future Steam evidence-base contract family is `srv_game_explore_period_metrics` / `/games/explore/overview`. This is not a current Steam endpoint/service/view implementation as `Combined`, not current `Combined` runtime lineage, ranking/KPI/score/recommendation source, or runtime contract change.
- `srv_chzzk_category_game_mapping` is the future backend identity input candidate for `Combined`. `GET /chzzk/category-game-mappings` remains a read-only inspection/API surface, not a backend-internal dependency when the DB serving view is available. Chzzk viewer/channel metrics remain bounded observed category evidence and must not be merged into `Combined` product semantics before a later approved implementation ticket.

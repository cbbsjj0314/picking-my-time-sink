...
Chzzk `fact_chzzk_category_30m` 은 현재 provider-specific DDL/parser candidate일 뿐이다.
아직 Postgres runtime write/integration path, live job boundary, serving read model, API, UI, canonical game mapping lineage가 없다.

...
local/private `channel-result.jsonl` artifact-to-Postgres write path로 승격되었고, API/web serving에서는 matching category observed buckets 안의 `unique_channels_observed` nullable metric에만 사용한다.
...
Chzzk `/chzzk/categories/overview` 는 `fact_chzzk_category_30m` 만 읽는 category overview API surface이며, optional `fact_chzzk_category_channel_30m` evidence가 있으면 matching category observed buckets 안에서 `unique_channels_observed = COUNT(DISTINCT channel_id)` 를 nullable observed metric으로 노출한다. Optional channel fact relation 또는 matching evidence가 없으면 null로 둔다.
`bounded_sample_caveat="bounded_sample"` 은 bounded

...
이 view는 API exposure, web exposure, product serving behavior, 또는 `Combined` semantics를 추가하지 않는다.

Candidate rows는 자동 populate되지 않으며, 현재 local candidate 17개도 이 ticket에서 insert하거나 promote하지 않는다.

Trusted insert, candidate-to-trusted promotion, API/web exposure, serving semantics, and `Combined` semantics는 별도 Human Gate controlled slice로 남는다.

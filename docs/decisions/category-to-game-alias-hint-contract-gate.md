...
Future synthetic/test-only implementation은 conceptually 아래 fields만 입력으로 고려할 수 있다.
...
Future implementation이 synthetic IDs를 tests에서 사용하더라도, 그 IDs는 synthetic/test-only fixtures일 뿐 real canonical identity claim이 아니다.
...
- Alias must not be inferred from raw provider payloads, raw API responses, or private runtime evidence.
...
어떤 proposal도 canonical game identity, hidden join, ranking/sorting/KPI, API response, web UI behavior, or `Combined` semantics에 사용할 수 없다.
